...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/07 16:43:50 by kbovt             #+#    #+#             */
/*   Updated: 2017/03/07 16:43:52 by kbovt            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_stack		*get_stack_file(t_ch_bskt **bskt)
{
	...
	del_tab(&tab_stack);
	if (is_dublic_stack(stack))
		return ((t_stack *)del_stack(&stack));
	close(fd);
	return (stack);
}

t_stack		*get_stack_argv(int argc, char **argv, t_ch_bskt **bskt)
{
	t_stack		*stack;
